...
| **Übungs-Katalog** | Umgesetzt in `lib/exerciseCatalog.ts`. Kanonische Übung (Name, Aliase DE/EN, Equipment). Freitext-Erfassung ist zu: Wizard und „Temporär anpassen“ nehmen nur Katalog-Treffer. `parseWodExercises` mappt die 708 lokalen WODs (Schema-Zeilen werden verworfen). Frames und YouTube-Id folgen mit den Grafiken. Mobilität bleibt bei `stretching_exercises`. |
| **Training-Suche Equipment** | Umgesetzt. Wörter wie Kurzhantel/Klimmzug werden Equipment bzw. Übung. Anzahl: Beliebig, 1, 2, 3, Mehr (4+). Körpergewicht zählt nicht, ein Kurzhantel-Paar ist ein Typ. Anzahl 1 heißt nur dieses Gerät. Ort-Kacheln sind aus, solange Equipment in der Suche steht; Tippen auf eine Kachel löscht die Equipment-Wörter. |
