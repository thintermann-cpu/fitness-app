...
| **Session AX** | **WOD Detail: Übungen lesbar + Session-Anpassung** — `splitWodExercises` macht Katalog-Blobs zur nummerierten Liste. Dauer ± und Übungen (Textarea, eine Zeile = eine Übung) gelten nur für die offene Session (Timer nutzt die Werte). Optional „Als eigenes Workout speichern“ → `custom_workouts` als `{name} (angepasst)` (Upsert bei erneutem Speichern). Katalog-DB/`wods.json` unverändert. Sichtbarkeit: 287 `is_visible=true` (Migration 022: CrossFit außer Girls/Heroes hidden); 369 bei Home = lokales JSON + Location-Filter, nicht Live. |
