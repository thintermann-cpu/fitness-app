...
| **Session BA** | **Formate, Sätze, Ein-Übungs-Workouts** — `presentCatalogWorkout` liest den ganzen Katalog (auch `is_visible=false`). Reps wie `3/1/3/1` hängen an der passenden Übung. `4×8 / 4×10` und „je Übung“ werden Kraft mit Sätzen und starten im Kraft-Timer. CrossFit ausser Girl/Hero wandert in HIIT (ohne Last), Kraft - Wenig Zeit (Last, bis 15 Min) oder Kraft-Ausdauer (Last, länger). Workouts mit nur einer Übung (Karen, Burpees, KB Grace) fallen aus der Liste. Home Gym bleibt als Kategorie-Label, 171 von 187 bleiben. |
