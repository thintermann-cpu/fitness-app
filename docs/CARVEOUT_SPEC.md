...
│   ├── sessionStore.ts        # Zustand-Store (kein persist): activeSession: { pillar, startedAt, meta? } | null; startSession(pillar, meta?)/endSession()/isSessionActive(pillar); genutzt von TimerView + GuidedSession + AdHocMeditationTimer um laufende Session zu tracken
...
│   ├── home/
│   │   ├── TodayPillarTracker.tsx  # 4 Chips (Done/Open) aus useTodayPillars; dreisprachig; Header-Label: "Aktueller Stand von heute · N von 4" (de/en/es); Chip-Reihenfolge: Routine · Workout · Stretching · Meditation; Chip-Tap navigiert direkt zur Pillar-Route (useNavigate)
...
│   │   ├── FreeTimerWizard.tsx  # Wizard; variant='save' (3 Steps: Modus → Übungen → Konfiguration/Name, speichert via customWorkouts.ts) | variant='adhoc' (4 Steps: Modus → Übungen → Konfiguration → Warmup-Frage); onStart(mode, minutes, withWarmup?, kraftConfig?, exercises?: WizardExercise[]) → triggert TimerView; bei nicht-Kraft-Modi werden exercises übergeben (wenn nicht leer); Dauer 1–60 min (AMRAP/ForTime); Modus-Auswahl via TIMER_LABELS aus timerLabels.ts; **Step 2 Slider**: AMRAP/ForTime zeigen neben +/−-Buttons einen `<input type="range" min=0/1 max=60>` mit `accentColor` der Modus-Farbe; gespeicherte Workouts erscheinen in CustomWorkoutsPage
...
│   ├── useWodHistory.ts       # localStorage + Supabase Dual-Write, personalBest; INSERT payload enthält user_id aus getSession(); onSuccess: setQueryData(['wod_history', '_all'] + ['wod_history', wod_name]) für sofortige Cache-Aktualisierung; **_fromLocal-Pattern**: bei lokalem Fallback wird `{ _fromLocal: true }` an Entry gehängt → `invalidateQueries` wird übersprungen (verhindert Supabase-Refetch der leere Cache überschreibt); Duplikat-Guard in setQueryData (`.filter(e => e.id !== data.id)` vor Prepend)
...
*Letzte Aktualisierung: Mai 2026 — Tim (Sessions AJ–AK: Auth-Race-Fixes, WarmupTimer-Übungsliste, Stretching-Robustheit, FreeTimerWizard-Slider)*
