...
| **Übungs-Katalog** | Umgesetzt in `lib/exerciseCatalog.ts`. Kanonische Übung (Name, Aliase DE/EN, Equipment). Der Übungsname ist kein Freitext. Ablauf (z. B. „3 Runden · 21-15-9“) und die Angabe pro Übung (Reps, Distanz) bleiben erhalten und sind editierbar. Weicht die Rundenzahl im Übungstext vom Feld `runden` ab, gilt der Text. Frames und YouTube-Id folgen mit den Grafiken. Mobilität bleibt bei `stretching_exercises`. |
