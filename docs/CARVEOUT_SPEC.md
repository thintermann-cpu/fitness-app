...
|---|---|---|
| **Workout** | `#E8642A` Orange | WOD-Datenbank (708 lokale JSON / live 905 total, **287 `is_visible=true`**), Timer (AMRAP/ForTime/EMOM/Tabata), Krafttraining-Modus (Satz-basierter Flow mit Gewichts-/Rep-Tracking), History, Highscores |
...
│   ├── customWorkouts.ts      # CustomWorkout + CustomSession Typen; localStorage-Fallback-Funktionen: `loadLocalWorkouts` / `saveLocalWorkout` / `deleteLocalWorkout` (Key: carveout_custom_workouts); `equipment?: string[]` liegt im Supabase-`config`-JSON (keine neue Spalte); CustomSession bleibt localStorage-only
│   ├── wodExercises.ts        # `splitWodExercises(text)`: Katalog-Freitext → lesbare Zeilen (· / ; / Nummerierung / Komma-Listen, Ranges wie `21-15-9` bleiben eine Zeile)
...
│   ├── HomePage.tsx               # Dashboard: Greeting + i18n-Datum, OnboardingSlides-Overlay, WhatsNewBanner, AdaptiveSuggestion (nur wenn `carveout_show_motivation !== 'false'`), TodayPillarTracker, MoodCheck (useDailyLog), Tabs Routinen/To-Do/Woche. Kein TodaysWod — der steht nur im WODs-Tab. **WeekStrip**; `showMotivation` als `useState(() => localStorage.getItem(...) !== 'false')` (robuster als direkter Aufruf); WeekStats + RecentActivity entfernt
...
│   ├── WorkoutPage.tsx        # Tabs: WODs / Timer / History. WODs-Tab: Workout des Tages, Meine Workouts + Neues Workout, Standort-Dropdown (Alle/Home/Gym/Bodyweight/Outdoor), dann Suche und Liste. Karten p-3, Listenabstand 8px. (früher: Workouts-Label, Location-Kacheln); Kategorie-Chips (Alle/CrossFit/HIIT/Kraft-Ausdauer/Kraft - Wenig Zeit/Krafttraining) als `flex flex-wrap` (kein overflow-x-scroll) über WodList; `wodCategory`-State → WodList-Prop; **Equipment-Filter**: `profile.equipment` → `userEquipment`; Location-Kacheln → `equipmentFilter`. Toggle "Alle anzeigen" (`showAllEquipment`) setzt `effectiveLocation` auf null (Kacheln unselektiert, beide Filter aus). Location-Klick bei aktivem Toggle schaltet den Toggle aus und wählt die Kachel. Timer-Tab idle-Zustand öffnet FreeTimerWizard (variant=adhoc) via "Timer konfigurieren"-Button; Krafttraining-Tab öffnet KrafttrainingView; timerConfig enthält exercises?: WizardExercise[]; handleWizardStart/handleAdhocStart nehmen 5. Param exercises auf; TimerView bekommt exercises={timerConfig.exercises}; `onShowHistory={() => setTab('history')}` an TimerView weitergegeben; Eigene-Tab zeigt CustomWorkoutsPage; **Tab-Sperre während Warmup**: `showWarmupTimer`-State sperrt Tab-Wechsel; `handleAdhocStart` setzt aktiven Tab explizit auf 'timer' (verhindert TimerView-Unmount + unterbrochene adHocLog-Kette)
...
│   ├── ProfilePage.tsx        # Datei noch im Repo, **nicht geroutet**. `/profile` → `/settings` inkl. Query (`?checkout=success`). CheckoutSuccess/Cancel-Pages existieren nicht.
...
│   │   ├── WodDetail.tsx      # enthält FavoriteButton (contentType="wod", color="#E8642A"); Detailansicht ohne Warmup-Akkordeon, ohne Warmup-Knopf und ohne Scaling; Warmup bleibt im Start-Wizard; nutzt WOD_TYPE_TO_MODE aus timerLabels.ts; feuert `carveout:workout-completed` CustomEvent bei Timer-Ende; `setShowScore(true)` aus `onComplete` entfernt — Completion-Screen wird von TimerView selbst gehandelt; **adHocLog immer aktiv**: `adHocLog` und `workoutName` werden bedingungslos an `TimerView` übergeben — jeder Timer-Start in `WodDetail` (direkt oder via Warmup) erzeugt einen `wod_history`-Eintrag mit dem WOD-Namen; `startedViaWarmup`-State entfernt; **Custom-Workout-Ansicht (`!wod`-Zweig)**: wenn `wodName` keinem DB-/JSON-WOD entspricht, wird via `useCustomWorkouts` nach passendem Custom Workout gesucht (`name === wodName`); bei Treffer: Custom-Workout-Ansicht (Name, Modus-Badge, Übungsliste); Start-Button startet `TimerView` mit gespeicherten `exercises`; `adHocLog` + `exercises` werden wie im Standard-Pfad an `addEntry.mutate()` übergeben (History-Augmentierung); **Session-Anpassung (Katalog)**: nummerierte Übungsliste; „Temporär anpassen“ (Dauer ±, Textarea) nur session-lokal; Timer: ForTime-Cap = Session-Minuten wenn > 0, AMRAP = Minuten, EMOM/Tabata-Länge aus den Minuten; „Bei eigenen speichern“ → `custom_workouts` `{name} (angepasst)` inkl. Equipment im config JSON
│   │   ├── TimerView.tsx      # Nutzt timer.worker.js; AMRAP/ForTime/EMOM/Tabata konfigurierbar; adHocLog-Prop: auto-Log in wod_history ohne WOD aus DB; CountdownOverlay: 3-2-1-Go Einblendung vor Timer-Start (SVG-Puls-Animation); exercises-Prop (WizardExercise[]): zeigt Übungsliste unterhalb Timer-Controls (Nummer, Name, optional Detail); Reset-useEffect prüft `isComplete` — kein Reset nach Timer-Ende (Restart-Bug-Fix); **EMOM/Tabata Übungsrotation**: aktueller Übungsname prominent + `NextExercisePreview` (EMOM: letzte 10s des Intervalls; Tabata Work: letzte 10s; Tabata Rest: gesamte Phase); **autoStart**: wenn `initialMode` gesetzt → `autoStart=true` → Config-UI + Modus-Selektor ausgeblendet, Countdown startet direkt beim Mount (useEffect on []); **Beep 10s vor Ende**: Web Audio API, kurzer Warnton (880 Hz) bei `timeLeft === 10` (nur ForTime/AMRAP); **Completion-Screen**: `isComplete` → eigener Screen (Emoji, "Gut gemacht!" / "Well done!", Score, Buttons: Schliessen + "In History anzeigen"); `onShowHistory`-Prop; `isComplete` in `setSessionActive` eingeschlossen (Swipe auch während Completion-Screen gesperrt); **exercises-Logging**: übergibt `exercises` an `addEntry.mutate()` wenn `exercises.length > 0` (Session AR)
│   │   ├── ExerciseInfoButton.tsx # `i` in der Übungszeile; Sheet mit Name, Angabe und Equipment aus dem Katalog. Kein Coaching-Text, keine Frames.
│   │   ├── FreeTimerWizard.tsx  # Wizard; variant='save' (3 Steps: Modus → Übungen → Konfiguration/Name, speichert via customWorkouts.ts) | variant='adhoc' (4 Steps: Modus → Übungen → Konfiguration → Warmup-Frage); onStart(mode, minutes, withWarmup?, kraftConfig?, exercises?: WizardExercise[], workoutName?, timerConfig?) → triggert TimerView; bei nicht-Kraft-Modi werden exercises übergeben (wenn nicht leer); Modus-Auswahl via TIMER_LABELS aus timerLabels.ts; **Step 2 Slider**: AMRAP/ForTime: `<input type="range" min=0/1 max=60>` neben +/−-Buttons; **Tabata Runden-Slider**: range 1–20 neben +/−-Buttons; **EMOM Runden-Slider**: range 1–30 neben +/−-Buttons; alle Slider mit `accentColor` der Modus-Farbe; `TimerInitConfig` Interface (tabataWork/tabataRest/tabataRounds/emomInterval/emomRounds); gespeicherte Workouts erscheinen in CustomWorkoutsPage; **onSaveOnly-Prop** (variant=adhoc): `onSaveOnly?: (config) => void` — "Nur speichern"-Button im letzten Schritt speichert Workout ohne Timer-Start; Name-Feld ist Pflichtfeld wenn `onSaveOnly` gesetzt; Button via `WizardShell.secondaryAction`
...
│   │   ├── CustomWorkoutsPage.tsx  # Schlanke Liste eigener Workouts; Suche „Name oder Equipment“ (Name, config.equipment, Übungsnamen); Zeile: Name + Meta links, Aktionen ▶/✎/✕ rechts; Name nur im Bearbeiten-Wizard, kein eigener Umbenennen-Button; nutzt `useCustomWorkouts`
│   │   ├── WodHistoryList.tsx  # zeigt Exercise-Namen als kompakte Zeile (`exercises.map(e => e.name).join(" · ")`) im History-Card wenn `entry.exercises?.length > 0` (Session AR)
│   │   └── ScoreInput.tsx
│   ├── routine/
...
│   │   └── WorkoutCountdown.tsx   # Wiederverwendbarer 3-2-1-Go Countdown-Overlay (SVG-Puls-Animation); Props: onComplete(); genutzt von WodDetail (Warmup-Pfad) + TimerView
│   ├── ui/
│   │   ├── Button.tsx
│   │   ├── Card.tsx
│   │   ├── Input.tsx
...
│   ├── useAudio.ts            # `playGong`/`playBeep`/`playComplete` weiterhin Web Audio API (Oszillator-Töne); **`startBackground`/`stopBackground` nutzen jetzt `<audio loop>` mit echten MP3-Dateien** (`AMBIENT_FILE`-Map → `public/audio/ambient/*.mp3`) statt generierter Web-Audio-Buffer; `SoundKey` erweitert um `fire` | `night` (7 Sounds + `silence`); isMuted-Check via audioStore in allen play*-Funktionen + startBackground
...
/                                → HomePage in AppShell (Begrüssung, Pillar-Stand, Mood + Wasser, Tabs Routinen/To-Do/Woche). Workout des Tages nur im WODs-Tab.
  /onboarding
  /workout
  /workout/custom
  /workout/:wodName
  ...
  /history
  /settings
  /profile                      → Navigate to="/settings" + Query (kein eigener Render)
...
Stripe Checkout: keine In-App-Success/Cancel-Pages. `useSubscription.startCheckout` sendet `success_url` = `{origin}/settings?checkout=success`. Edge-Function-Default ebenfalls `/settings?checkout=success`. `/profile` leitet Query mit. `SettingsPage` zeigt Toast + Abo-View bei `?checkout=success`. Edge Function muss nach Deploy neu ausgerollt werden, sonst gilt der Client-Body trotzdem.
...
| `wod_history` | Workout-Logs pro Nutzer (WOD, Score, Datum, Notizen); **Migration 029**: `exercises` JSONB NOT NULL DEFAULT '[]' — speichert Übungsliste aus Custom-Workout-Session |
...
| `push_preferences` | Reminder-Einstellungen (morning/evening/wod/inactivity + Zeiten); Migration `030_push_preferences.sql`, RLS mit USING und WITH CHECK |
...
**Migration 029** (`supabase/migrations/029_wod_history_exercises.sql`): `ALTER TABLE wod_history ADD COLUMN IF NOT EXISTS exercises JSONB NOT NULL DEFAULT '[]'`
...
Zugangsbedingung: `user_profiles.role IN ('admin', 'moderator')`, geprüft von `AdminRoute.tsx`, erst nachdem das Profil geladen ist. `onAuthStateChange` fragt das Profil erst nach dem Callback ab, damit ein Refresh nicht am Auth-Lock hängen bleibt. Die Login-Seite bleibt sichtbar, während die Session lädt, und der Login wartet nicht auf das Profil. Ein Refresh zeichnet sofort mit der gespeicherten Anmeldung. Der Browser-Lock wird nicht verwendet. REST-Abfragen nehmen das gespeicherte Access-Token, damit Profil, Equipment und Favoriten nicht leer wirken. Kommt `getSession()` später an, werden Profil und Favoriten nachgeladen. Eine leere Favoriten-Antwort überschreibt den lokalen Stand nicht. Auth-Antworten cached der Service Worker nicht.
...
WODs (708 lokal / live 905 total, 287 sichtbar) aktuell nur Deutsch — Übersetzungen EN/ES offen (siehe Roadmap).
...
| **Session AQ** | **WorkoutCountdown + Musik-Links + FreeTimerWizard SaveOnly + WodDetail Warmup-History + useWods Filter-Fix** — (1) **WorkoutCountdown.tsx** (neue shared-Komponente): wiederverwendbarer 3-2-1-Go Countdown-Overlay (SVG-Puls-Animation), Props: `onComplete()`; genutzt von WodDetail (Warmup-Pfad) + TimerView; (2) **Musik-Links in Settings**: neues Training-Submenu-Subsection "Musik-Links" mit 3 Text-Inputs (localStorage Keys: `carveout_music_workout`, `carveout_music_stretching`, `carveout_music_meditation`); Musik-Button in `GuidedSession` (Stretching) + `MeditationSession` öffnet gespeicherten Link in neuem Tab; (3) **FreeTimerWizard `onSaveOnly`**: neuer optionaler Prop — "Nur speichern"-Button im letzten Schritt über `WizardShell.secondaryAction`; speichert Custom Workout ohne Timer-Start; Name ist Pflichtfeld wenn `onSaveOnly` gesetzt; (4) **WodDetail adHocLog via Warmup**: nach WarmupTimer-Ende → WorkoutCountdown → TimerView mit `adHocLog`-Prop → History-Eintrag für Warmup-initiierte Sessions; (5) **useWods `applyLocalFilters`**: `wodCategory`-Filter für lokales JSON (wods.json) explizit übersprungen — lokales JSON hat keine `wod_category`-Felder; `WizardShell`: `secondaryAction`-Prop ergänzt |
| **Session AQ-followup** | **Custom Workout Flow Bugfixes** — (1) **Stale-Ref-Fix (`WorkoutPage`)**: neuer `timerKey`-State (Counter); `TimerView` bekommt `key={timerKey}` → erzwungenes Remount bei jedem neuen Timer-Start (verhindert Stale-State/Refs von vorherigem Lauf); `timerKey` wird in `handleWizardStart`, `handleAdhocStart` und `handleStartSaved` inkrementiert; (2) **Exercises für gespeicherte Workouts**: `handleStartSaved` übergibt `exercises: w.mode !== 'krafttraining' ? w.exercises : undefined` an `timerConfig` — Übungsliste aus Custom Workout war bisher leer beim Start über `CustomWorkoutsPage`; (3) **Warmup für gespeicherte Workouts**: `handleStartSaved` ruft `setShowWarmupTimer(true)` — gespeicherte Workouts starten jetzt konsistent mit Warmup-Flow (wie Wizard-Start über `handleWizardStart`) |
...
| **Session BB** | **Runden- und Rep-Zähler im Timer** — Bei For Time und AMRAP liegt unter der Uhr „Runde fertig“ plus Reps +/−. Steht im Ablauf „N Runden“, gibt es bis 12 abhakbare Kreise (10 Rounds Grind). Eine fertige Runde setzt die Reps auf 0 und springt zur ersten Übung. AMRAP speichert den Score als `Runden` bzw. `4+12`, For Time bleibt die Zeit und die Zählung steht in den Notizen. Während der Session sind App-Header und Bottom-Nav ausgeblendet, die aktuelle Übung steht gross. Pfeile daneben schalten bei For Time und AMRAP zur vorherigen oder nächsten Übung, „Runde fertig“ setzt auf die erste. Die Liste darunter bleibt kleiner, aber lesbar. Der laufende Screen füllt die Höhe und verteilt den freien Platz zwischen Übung, Uhr, Zähler und Liste. Ist der Inhalt höher als der Screen, schrumpfen die Abstände und die Liste scrollt. Ein `i` in der Zeile öffnet ein Sheet (Name, Angabe, Equipment). Kurztext und Frames folgen. |
...
| **Session BF** | **Workout des Tages nur im Training** — `TodaysWod` rendert nur noch im WODs-Tab von `WorkoutPage`. Mein Tag (`HomePage`) zeigt die Kachel nicht mehr. |
| **Session BE** | **Versand und Admin** — `send-dispatch` schickt Web-Push an die Abos (Erinnerung nach `push_preferences`, Broadcast an alle). E-Mail über Resend ist davon getrennt: Newsletter, Zahlung, Konto, nie die Erinnerung. Morgen-Push nennt die heutigen Routinen. Die Uhrzeit gilt in der Stadt aus den Benachrichtigungen (`push_preferences.city` und `timezone`, Migration 032, Vorschlag via Open-Meteo). Ohne Stadt: Europe/Zurich. Secrets: `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `RESEND_API_KEY`. Admin-Seiten Push, Emails, Feedback ersetzen die Platzhalter. Ein Pillar-Abschluss hakt heutige Routinen mit passendem `linked_pillar` ab. `routine_logs` werden beim Öffnen und beim Zurückkehren neu geladen. |
| **Session BD** | **Kachel bleibt nach dem Workout** — Jeder Abschluss (Ad-hoc, eigenes, Katalog, Kraft) schreibt das Workout lokal in `carveout_pillar_done` und in `wod_history`. `useTodayPillars` und `useWeekPillars` lesen den lokalen Stand mit, `staleTime: 0`, Reload und Fokus laden neu. Auf dem Fertig-Screen steht „Cooldown · Mobilität“ und führt nach `/stretching`. Migration `030_push_preferences.sql` schreibt die Push-Einstellungen fest. |
| **Session BC** | **Workout-Detail aufgeräumt** — In der Ansicht vor dem Start sind das Warmup-Akkordeon, der Warmup-Knopf neben Start und der Scaling-Block weg. Anpassen, Equipment und Start bleiben. Warmup gibt es weiter im Start-Wizard. |
...
| **Übungs-Katalog** | Umgesetzt in `lib/exerciseCatalog.ts`. Kanonische Übung (Name, Aliase DE/EN, Equipment). Der Übungsname ist kein Freitext. Steht bei jeder Übung dieselbe Anzahl vorne im Namen (`10 Curl · 10 Row`), wandert die Zahl ins Reps-Feld und der Name bleibt ohne Zahl. Ablauf (z. B. „3 Runden · 21-15-9“) und die Angabe pro Übung (Reps, Distanz) bleiben erhalten und sind editierbar. Weicht die Rundenzahl im Übungstext vom Feld `runden` ab, gilt der Text. „Start Timer“ und der Start eines eigenen Workouts öffnen zuerst den Timer-Wizard (Übungen vorausgefüllt, dann Dauer, dann Warm-up). Warm-up hat drei Wahlen: Keins, Kurz (3 min: Armkreisen, Inchworms, Ausfallschritte, Jumping Jacks, Liegestütze) und Standard (4 min). Der Countdown startet erst danach. Frames und YouTube-Id folgen mit den Grafiken. Mobilität bleibt bei `stretching_exercises`. |
| **Training-Suche Equipment** | Umgesetzt. Wörter wie Kurzhantel/Klimmzug werden Equipment bzw. Übung. Anzahl: Beliebig, 1, 2, 3, Mehr (4+). Beliebig plus genanntes Gerät heißt: dieses Gerät ist dabei, weitere Geräte bleiben offen. Körpergewicht zählt nicht, ein Kurzhantel-Paar ist ein Typ. Anzahl 1 heißt nur dieses Gerät. Ort-Kacheln sind aus, solange Equipment in der Suche steht. |
...
| **WOD-Übersetzungen** | EN/ES für den Katalog (aktuell nur DE; 708 lokal / 905 total / 287 sichtbar) |
...
| **Migration ohne Nummer** | `add_push_subscriptions.sql` + `add_role_and_admin_rls.sql` auf Nummern-Präfix umstellen. `push_preferences` liegt in `030_push_preferences.sql`. |
...
- Narration via OpenAI TTS API (~$2 für alle ~90 Clips)
...
| **Stripe Return-URL verloren** | `success_url` `/profile?checkout=success` → Redirect `/settings` ohne Query | Client + Edge-Default `/settings?checkout=success`; `/profile` leitet Query mit; Settings-Toast | Branch `cursor/fix-stripe-dice-sidebar-toggle-a5fc` |
| **Würfel nur lokales JSON** | `pickRandomWod` las `wods.json` — Programm-WODs nicht im Pool | `fetchMatchingWods` live Supabase, gleiche Filter wie die Liste | Branch `cursor/fix-stripe-dice-sidebar-toggle-a5fc` |
| **Sidebar nested links + Icon** | `<a>` in `<a>` (`/profile` um `/settings`); Achtsamkeit 🧠 | Ein Link `/settings`; Icon 🧘 in Sidebar + BottomNav | Branch `cursor/fix-stripe-dice-sidebar-toggle-a5fc` |
| **Equipment-Toggle vs Home-Kachel** | „Alle anzeigen“ änderte nur den Text, Home blieb orange und filterte weiter | `effectiveLocation` null bei `showAllEquipment`; Location-Klick reaktiviert den Filter | Branch `cursor/fix-stripe-dice-sidebar-toggle-a5fc` |

---

*Letzte Aktualisierung: September 2026 — Session BF: Workout des Tages nur im WODs-Tab*
