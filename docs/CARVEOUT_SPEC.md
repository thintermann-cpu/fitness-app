...
│   ├── HomePage.tsx               # Dashboard: Greeting + i18n-Datum, OnboardingSlides-Overlay, WhatsNewBanner, AdaptiveSuggestion (nur wenn `carveout_show_motivation !== 'false'`), TodayPillarTracker, WeekStrip, MoodCheck (useDailyLog), TodaysWod; WeekStats + RecentActivity entfernt
...
│   ├── useDailyLog.ts         # Tages-Mood, Wasser; **auth-gated**: `enabled: !!userId`; queryKey `['daily_log', userId ?? 'anon', date]`; userId aus Store
...
└── public/
    ...
    ├── onboarding-slides.json  # 5 Produkttour-Slides (id/emoji/title/subtitle/pillarColor); JSON-Datei, von OnboardingSlides.tsx geladen
...
*Letzte Aktualisierung: Mai 2026 — Tim (Session AO: OnboardingSlides, WhatsNewBanner, Mood-adaptive TodaysWod, showMotivation-Toggle)*
