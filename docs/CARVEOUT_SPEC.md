...
│   ├── RoutinePage.tsx        # Titel: Rituale; Tabs: Rituale / Todo / Woche (kein WaterTracker, kein MoodCheck); hört auf `carveout:workout-completed` CustomEvent — auto-completed offene Todos deren Text `/workout|training|sport/i` matcht
...
│   │   ├── TimerView.tsx      # Nutzt timer.worker.js; AMRAP/ForTime/EMOM/Tabata konfigurierbar; adHocLog-Prop: auto-Log in wod_history ohne WOD aus DB; CountdownOverlay: 3-2-1-Go Einblendung vor Timer-Start (SVG-Puls-Animation); exercises-Prop (WizardExercise[]): zeigt Übungsliste unterhalb Timer-Controls (Nummer, Name, optional Detail); Reset-useEffect prüft `isComplete` — kein Reset nach Timer-Ende (Restart-Bug-Fix)
...
│   │   ├── GuidedSession.tsx   # (bestehend) + `defaultExerciseDuration`-Prop (überschreibt config-Default, genutzt von Yoga Flows mit flow.holdTime); Yoga-Atemhinweis (subcategory=yoga_flow); ExerciseKeyframes integriert (Bild-Crossfade pro Übung); per-exercise `duration_sec` überschreibt globales `exerciseDuration` (Fallback wenn 0); Next-Up-Banner: zeigt nächste Übung wenn `phase === 'exercise' && timeLeft <= 10`
...
*Letzte Aktualisierung: Mai 2026 — Tim (Session T: Timer-Restart-Fix, Equipment-Filter-Fix, AmbientPlayer, Ritual Auto-Check, per-exercise duration, Next-Up-Banner)*
