...
| **Session BE** | **Versand und Admin** — `send-dispatch` schickt Web-Push an die Abos (Erinnerung nach `push_preferences`, Broadcast an alle). E-Mail über Resend ist davon getrennt: Newsletter, Zahlung, Konto, nie die Erinnerung. Morgen-Push nennt die heutigen Routinen. Zeiten in Europe/Zurich. Secrets: `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `RESEND_API_KEY`. Admin-Seiten Push, Emails, Feedback ersetzen die Platzhalter. Ein Pillar-Abschluss hakt heutige Routinen mit passendem `linked_pillar` ab. `routine_logs` werden beim Öffnen und beim Zurückkehren neu geladen. |
